...
public class MapObjectList {
    ...
    public final BufferedImageIcon[] getAllGameAppearances() {
	final BufferedImageIcon[] allGameAppearances = new BufferedImageIcon[this.allObjects.length];
	int x;
	for (x = 0; x < this.allObjects.length; x++) {
    ...
    public final BufferedImageIcon[] getAllEditorAppearances() {
    ...
    public final BufferedImageIcon[][] getAllOtherAppearances() {
	final BufferedImageIcon[][] allOtherAppearances = new BufferedImageIcon[this.allObjects.length][];
	int x, y;
	for (x = 0; x < this.allObjects.length; x++) {
	    final int otherAppearanceCount = this.allObjects[x].getOtherAppearanceCount();
	    allOtherAppearances[x] = new BufferedImageIcon[otherAppearanceCount];
	    for (y = 0; y < otherAppearanceCount; y++) {
		allOtherAppearances[x][y] = this.allObjects[x].getOtherAppearance(y);
	    }
	}
	...
	int count = 0;
	for (x = 0; x < this.allObjects.length; x++) {
	    if (!this.allObjects[x].hasPrerequisite() && !this.allObjects[x].isRequired()) {
		tempAllWithoutPrereq[count] = this.allObjects[x];
		count++;
	    }
	}
