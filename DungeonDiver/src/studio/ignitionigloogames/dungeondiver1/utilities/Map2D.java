...
public class Map2D implements NDimensionalMap {
    ...
    // Constructors
    public Map2D(final int rows, final int cols, final Dimension viewingWindowDimensions, final MapObject errorObject) {
	...
	this.view.setLayout(new GridLayout(this.viewingWindowSize.width, this.viewingWindowSize.height));
    ...
    @Override
    public MapObject getCellOffset(final NDimensionalLocation loc, final NDimensionalLocation offset,
	    MapObject defaultValue) {
	final int row = loc.getLocation(NDimensionalMap.ROW_DIMENSION)
		+ offset.getLocation(NDimensionalMap.ROW_DIMENSION);
	final int col = loc.getLocation(NDimensionalMap.COLUMN_DIMENSION)
		+ offset.getLocation(NDimensionalMap.COLUMN_DIMENSION);
	if (this.mapData != null && col >= 0 && col < this.mapData.length && this.mapData[col] != null && row >= 0
		&& row < this.mapData[col].length && this.mapData[col][row] != null) {
	    return this.mapData[col][row];
	}
	return defaultValue;
    }
	...
	final NDimensionalLocation loc = new NDimensionalLocation(this.getMaxDimension());
	...
	for (x = 0; x < columns; x++) {
		...
		if (this.mapData[x][y].getName().equals(obj.getName())) {
		    // Found it
		    this.findResult.setLocation(NDimensionalMap.COLUMN_DIMENSION, x);
		    this.findResult.setLocation(NDimensionalMap.ROW_DIMENSION, y);
		    return true;
		}
	    }
	}
    ...
    @Override
    public boolean findObjectOnDimension(final int dimension, final MapObject obj) {
    ...
    @Override
    public boolean findNthObjectOnDimension(final int dimension, final int N, final MapObject obj) {
    ...
    @Override
    public void setStart(final int dimension, final NDimensionalLocation newStart) {
	final int row = newStart.getLocation(NDimensionalMap.ROW_DIMENSION);
	final int column = newStart.getLocation(NDimensionalMap.COLUMN_DIMENSION);
    ...
    @Override
    public void fillMapRandomly(final MapObjectList objects, final MapObject pass1Fill) {
	// Pass 1
	...
	final int columns = loc.getLocation(NDimensionalMap.COLUMN_DIMENSION);
	final int rows = loc.getLocation(NDimensionalMap.ROW_DIMENSION);
	final MapObject[] objectsWithoutPrerequisites = objects.getAllWithoutPrerequisiteAndNotRequired();
	RandomRange r = new RandomRange(0, objectsWithoutPrerequisites.length - 1);
	int x, y;
	for (x = 0; x < columns; x++) {
	    for (y = 0; y < rows; y++) {
		final MapObject placeObj = objectsWithoutPrerequisites[r.generate()];
		final NDimensionalLocation loc2 = new NDimensionalLocation(this.getMaxDimension());
		...
		if (okay) {
			...
			this.mapData[x][y] = objects.getNewInstanceByName(placeObj.getName());
		    }
		}
	    }
	}
	// Pass 3...N
	int N = 1;
	MapObject[] objectsWithPrerequisites = objects.getAllWithNthPrerequisite(N);
	while (objectsWithPrerequisites != null) {
	    r = new RandomRange(0, objectsWithPrerequisites.length - 1);
	    for (x = 0; x < columns; x++) {
		for (y = 0; y < rows; y++) {
		    final MapObject placeObj = objectsWithPrerequisites[r.generate()];
		    final NDimensionalLocation loc2 = new NDimensionalLocation(this.getMaxDimension());
		    loc2.setLocation(NDimensionalMap.COLUMN_DIMENSION, x);
		    loc2.setLocation(NDimensionalMap.ROW_DIMENSION, y);
		    final boolean okay = placeObj.shouldGenerateObject(loc2, this);
	...
	final RandomRange row = new RandomRange(0, this.getDimensions().getLocation(NDimensionalMap.ROW_DIMENSION) - 1);
	final RandomRange column = new RandomRange(0,
		this.getDimensions().getLocation(NDimensionalMap.COLUMN_DIMENSION) - 1);
	final NDimensionalLocation loc3 = new NDimensionalLocation(this.getMaxDimension());
	...
	for (x = 0; x < requiredObjects.length; x++) {
		...
		if (currObj.shouldGenerateObject(loc3, this)) {
			...
			this.mapData[randomColumn][randomRow] = objects.getNewInstanceByName(currObj.getName());
		    }
		} else {
    ...
    @Override
    public void fillMapDimensionRandomly(final MapObjectList objects, final MapObject pass1Fill, final int dimension) {
	...
	this.view.removeAll();
	int x, y;
	final int maxViewingWindowRow = this.viewingWindowLocation.y + this.viewingWindowSize.height;
	final int maxViewingWindowColumn = this.viewingWindowLocation.x + this.viewingWindowSize.width;
	for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
	    for (y = this.viewingWindowLocation.y; y < maxViewingWindowRow; y++) {
		if (this.mapData != null && y >= 0 && y < this.mapData.length && this.mapData[y] != null && x >= 0
			&& x < this.mapData[y].length && this.mapData[y][x] != null) {
		    this.view.add(new JLabel(this.mapData[y][x].getGameAppearance()));
		} else {
    ...
    @Override
    public Container drawGameWithVisibility(final NDimensionalLocation otherDimensions) {
	this.view.removeAll();
	int x, y;
	final int maxViewingWindowRow = this.viewingWindowLocation.y + this.viewingWindowSize.height;
	final int maxViewingWindowColumn = this.viewingWindowLocation.x + this.viewingWindowSize.width;
	for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
		...
		if (this.mapData != null && y >= 0 && y < this.mapData.length && this.mapData[y] != null && x >= 0
			&& x < this.mapData[y].length && this.mapData[y][x] != null) {
		    if (this.isVisibleHorizontal(x) && this.isVisibleVertical(y)) {
			this.view.add(new JLabel(this.mapData[y][x].getGameAppearance()));
		    } else {
			this.view.add(new JLabel(this.hidden.getGameAppearance()));
		    }
		} else {
		    this.view.add(new JLabel(this.error.getGameAppearance()));
		}
	...
	for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
		...
		if (this.mapData != null && y >= 0 && y < this.mapData.length && this.mapData[y] != null && x >= 0
			&& x < this.mapData[y].length && this.mapData[y][x] != null) {
		    this.view.add(new JLabel(this.mapData[y][x].getEditorAppearance()));
		} else {
    ...
    @Override
    public Container drawEditorWithVisibility(final NDimensionalLocation otherDimensions) {
	this.view.removeAll();
	int x, y;
	final int maxViewingWindowRow = this.viewingWindowLocation.y + this.viewingWindowSize.height;
	final int maxViewingWindowColumn = this.viewingWindowLocation.x + this.viewingWindowSize.width;
	for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
		...
		if (this.mapData != null && y >= 0 && y < this.mapData.length && this.mapData[y] != null && x >= 0
			...
			this.view.add(new JLabel(this.hidden.getEditorAppearance()));
		    }
		} else {
		    this.view.add(new JLabel(this.error.getEditorAppearance()));
		}
    ...
    @Override
    public Container drawOther(final int appearanceID, final NDimensionalLocation otherDimensions) {
	this.view.removeAll();
	int x, y;
	final int maxViewingWindowRow = this.viewingWindowLocation.y + this.viewingWindowSize.height;
	final int maxViewingWindowColumn = this.viewingWindowLocation.x + this.viewingWindowSize.width;
	for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
		...
		if (this.mapData != null && y >= 0 && y < this.mapData.length && this.mapData[y] != null && x >= 0
			&& x < this.mapData[y].length && this.mapData[y][x] != null) {
		    this.view.add(new JLabel(this.mapData[y][x].getOtherAppearance(appearanceID)));
		} else {
		    this.view.add(new JLabel(this.error.getOtherAppearance(appearanceID)));
		}
	    }
	}
	return this.view;
    ...
    @Override
    public Container drawOtherWithVisibility(final int appearanceID, final NDimensionalLocation otherDimensions) {
	this.view.removeAll();
	int x, y;
	final int maxViewingWindowRow = this.viewingWindowLocation.y + this.viewingWindowSize.height;
	final int maxViewingWindowColumn = this.viewingWindowLocation.x + this.viewingWindowSize.width;
	for (x = this.viewingWindowLocation.x; x < maxViewingWindowColumn; x++) {
		...
		if (this.mapData != null && y >= 0 && y < this.mapData.length && this.mapData[y] != null && x >= 0
			...
			this.view.add(new JLabel(this.hidden.getOtherAppearance(appearanceID)));
		    }
		} else {
		    this.view.add(new JLabel(this.error.getOtherAppearance(appearanceID)));
		}
	...
	if (Math.abs(x - this.viewingWindowLocation.x - this.viewingWindowSize.width / 2) < this.drawDistance.width) {
	    return true;
	} else {
	    return false;
	}
	...
	if (Math.abs(y - this.viewingWindowLocation.y - this.viewingWindowSize.height / 2) < this.drawDistance.height) {
	    return true;
	} else {
	    return false;
	}
