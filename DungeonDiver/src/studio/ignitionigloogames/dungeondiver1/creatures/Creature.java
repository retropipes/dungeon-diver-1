...
import studio.ignitionigloogames.dungeondiver1.creatures.spells.SpellBook;
...
public abstract class Creature implements Serializable, StatConstants {
    ...
    public int getBuffRounds(final Buff b) {
        if (b != null) {
            int index = this.indexOf(b);
            if (index != -1) {
                return this.get(index).getRounds();
            }
        }
        return 0;
    }
    ...
    public String getBuffMessage(final Buff b, final int which) {
        if (b != null) {
            int index = this.indexOf(b);
            if (index != -1) {
                return this.get(index).getMessage(which);
            }
        }
        return null;
    }
    ...
    public String getBuffString(final Buff b) {
        if (b != null) {
            int index = this.indexOf(b);
            if (index != -1) {
                return this.get(index).getBuffString();
            }
        }
        return null;
    }
