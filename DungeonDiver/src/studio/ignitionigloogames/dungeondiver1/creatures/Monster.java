...
import studio.ignitionigloogames.dungeondiver1.creatures.ai.RandomAIRoutinePicker;
...
public class Monster extends Creature {
	...
	final RandomRange r2 = new RandomRange(0, this.level + player.getPower());
	return r1.generate() + r2.generate();
    }

    private int getInitialDefense() {
	final Player player = DungeonDiver.getHoldingBag().getPlayer();
	final RandomRange r1 = new RandomRange(0, player.getPermanentDefense());
	final RandomRange r2 = new RandomRange(0, this.level + player.getBlock());
	return r1.generate() + r2.generate();
    }
	...
	final long toNext = player.getExpToNextLevel(player.getLevel() + 1, player.getKills());
	final long toCurrent = player.getExpToNextLevel(player.getLevel(), player.getKills());
	final long needed = toNext - toCurrent;
	final long factor = Monster.getFightsPerLevel();
	final long exp = (long) ((needed / factor + r.generate()) * this.adjustForLevelDifference());
	if (exp < 0L) {
	    return 0L;
	} else {
	    return exp;
	...
	final int averageHealingCost = Shop.getHealingCost(player.getLevel(), player.getMaximumHP() * 2 / 3,
		player.getMaximumHP(), player.getKills());
	final int averageRegenerationCost = Shop.getRegenerationCost(player.getLevel(), player.getMaximumMP() / 2,
		player.getMaximumMP(), player.getKills());
	final int max = needed / factor * 2 + averageHealingCost + averageRegenerationCost;
	...
	return Monster.HP_PER_LEVEL * this.level + r1.generate() + r2.generate();
    }

    private int getInitialMaximumMP() {
	final Player player = DungeonDiver.getHoldingBag().getPlayer();
	final RandomRange r1 = new RandomRange(0, player.getPermanentMP());
	final RandomRange r2 = new RandomRange(0, this.level * Monster.MP_PER_LEVEL);
	return r1.generate() + r2.generate();
    }
	...
	final Player player = DungeonDiver.getHoldingBag().getPlayer();
	return (int) (Monster.BASE_FIGHTS_PER_LEVEL + (player.getLevel() - 1) * Monster.FIGHTS_PER_LEVEL_INCREMENT);
    }
