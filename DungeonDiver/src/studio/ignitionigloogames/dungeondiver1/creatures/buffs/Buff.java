...
public class Buff implements Serializable, StatConstants {
    ...
    public Buff(final String buffName, final int newRounds, final double rScaleFactor, final int rScaleStat) {
	this.name = buffName;
	this.messages = new String[Buff.MAX_MESSAGES];
	this.effect = new double[Buff.MAX_EFFECT_TYPES];
	this.initialEffect = new double[Buff.MAX_EFFECT_TYPES];
	...
	if (this.rounds == this.initialRounds) {
	    if (!this.messages[Buff.MESSAGE_INITIAL].equals(Buff.getNullMessage())) {
		msg += this.messages[Buff.MESSAGE_INITIAL] + "\n";
	    }
	}
	if (!this.messages[Buff.MESSAGE_SUBSEQUENT].equals(Buff.getNullMessage())) {
	    msg += this.messages[Buff.MESSAGE_SUBSEQUENT] + "\n";
	}
	if (this.rounds == 1) {
	    if (!this.messages[Buff.MESSAGE_WEAR_OFF].equals(Buff.getNullMessage())) {
	...
	if (Double.doubleToLongBits(this.effectDecayRate) != Double.doubleToLongBits(other.effectDecayRate)) {
	    return false;
	}
	...
	if (Double.doubleToLongBits(this.roundsScaleFactor) != Double.doubleToLongBits(other.roundsScaleFactor)) {
	    return false;
	}
