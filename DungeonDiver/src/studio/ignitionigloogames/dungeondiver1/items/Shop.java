...
public class Shop implements ShopTypes {
	...
	if (this.type == ShopTypes.WEAPONS || this.type == ShopTypes.ARMOR) {
		...
		stage1Choices[x] = ItemNames.getItemName(this.type, x + 1, player.getPlayerClass());
	    }
	...
	final String stage1Result = (String) JOptionPane.showInputDialog(null, this.getGoldTotals() + "Select",
		Shop.getShopNameFromType(this.type), JOptionPane.QUESTION_MESSAGE, null, stage1Choices,
	...
	if (this.type == ShopTypes.WEAPONS || this.type == ShopTypes.ARMOR) {
	    ...
	    cost = Shop.getHealingCost(player.getLevel(), player.getCurrentHP(), player.getMaximumHP(),
		    player.getKills());
	} else if (this.type == ShopTypes.REGENERATOR) {
	    cost = Shop.getRegenerationCost(player.getLevel(), player.getCurrentMP(), player.getMaximumMP(),
		    player.getKills());
	} else if (this.type == ShopTypes.BANK) {
	    String stage2Result = "";
	    while (stage2Result.equals("")) {
		stage2Result = JOptionPane.showInputDialog(null, this.getGoldTotals() + stage1Result + " How Much?",
			Shop.getShopNameFromType(this.type), JOptionPane.QUESTION_MESSAGE);
		...
		try {
		    ...
		    JOptionPane.showMessageDialog(null, "Amount must be greater than zero.",
			    Shop.getShopNameFromType(this.type), JOptionPane.INFORMATION_MESSAGE);
		    stage2Result = "";
		}
	    }
	}
	if (this.type != ShopTypes.BANK) {
	    final int stage2Confirm = JOptionPane.showConfirmDialog(null,
		    "This will cost " + cost + " Gold. Are you sure?", Shop.getShopNameFromType(this.type),
		    JOptionPane.YES_NO_OPTION);
	    if (stage2Confirm == JOptionPane.NO_OPTION || stage2Confirm == JOptionPane.CLOSED_OPTION) {
		return;
	    }
	}
	if (this.type == ShopTypes.WEAPONS) {
		...
		final int stage2SecondConfirm = JOptionPane.showConfirmDialog(null,
			"This purchase will downgrade your weapon.\n" + "Are you REALLY sure you want to buy this?",
			Shop.getShopNameFromType(this.type), JOptionPane.YES_NO_OPTION);
		if (stage2SecondConfirm == JOptionPane.NO_OPTION || stage2SecondConfirm == JOptionPane.CLOSED_OPTION) {
		    return;
		}
	    }
	...
	if (this.type == ShopTypes.BANK) {
		...
		if (player.getGold() < cost) {
		    JOptionPane.showMessageDialog(null, "Not Enough Gold!", Shop.getShopNameFromType(this.type),
