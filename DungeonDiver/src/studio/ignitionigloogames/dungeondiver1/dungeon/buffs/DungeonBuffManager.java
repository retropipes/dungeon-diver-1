...
public class DungeonBuffManager {
	...
	this.effectsPane.setLayout(new GridLayout(DungeonBuffManager.MAX_BUFFS, 1));
	...
	if (this.isConfused()) {
	    this.buffs[DungeonBuffManager.BUFF_CONFUSED].extendDungeonBuff(rounds);
	} else {
	    if (this.isDizzy()) {
	...
	if (this.isRotated()) {
	    final int currState = this.buffs[DungeonBuffManager.BUFF_ROTATED].getPower();
	    if (currState == state) {
		// Extend Rotated
		this.buffs[DungeonBuffManager.BUFF_ROTATED].extendDungeonBuff(rounds);
	    } else {
		// Clear Rotated
	    ...
	    // Set Rotated
	    this.buffs[DungeonBuffManager.BUFF_ROTATED] = new Rotated(rounds, state);
	}
    }
	...
	if (this.isShiningBrightly()) {
	    ...
	    this.savedRounds[DungeonBuffManager.BUFF_SHINING] = this.buffs[DungeonBuffManager.BUFF_SHINING].getRounds();
	    // Get rid of Shining
		...
		this.buffs[DungeonBuffManager.BUFF_PARTLY_BLINDED].customUseLogic(0);
	...
	if (this.isShiningBrightly()) {
	    this.buffs[DungeonBuffManager.BUFF_SHINING_BRIGHTLY].extendDungeonBuff(rounds);
	} else {
	    this.buffs[DungeonBuffManager.BUFF_SHINING_BRIGHTLY] = new ShiningBrightly(rounds);
	    DungeonDiver.getHoldingBag().getDungeonGUI().increaseVisibility();
	    DungeonDiver.getHoldingBag().getDungeonGUI().increaseVisibility();
	}
    }
	...
	if (this.isBlinded()) {
		...
		this.buffs[DungeonBuffManager.BUFF_SHINING_BRIGHTLY].setRounds(0);
		this.buffs[DungeonBuffManager.BUFF_SHINING_BRIGHTLY].customUseLogic(0);
		...
		this.buffs[DungeonBuffManager.BUFF_PARTLY_BLINDED] = new PartlyBlinded(rounds);
	...
	if (this.isBlinded()) {
	    this.buffs[DungeonBuffManager.BUFF_BLINDED].extendDungeonBuff(rounds);
	} else {
	...
	if (this.isDizzy()) {
	    if (this.isRotated()) {
		final int diz = this.buffs[DungeonBuffManager.BUFF_DIZZY].useDungeonBuff(arg);
		return this.buffs[DungeonBuffManager.BUFF_ROTATED].useDungeonBuff(diz);
	    } else {
		return this.buffs[DungeonBuffManager.BUFF_DIZZY].useDungeonBuff(arg);
	    }
	} else {
	    if (this.isConfused() && this.isRotated()) {
		final int conf = this.buffs[DungeonBuffManager.BUFF_CONFUSED].useDungeonBuff(arg);
		return this.buffs[DungeonBuffManager.BUFF_ROTATED].useDungeonBuff(conf);
	    } else if (this.isConfused() && !this.isRotated()) {
		return this.buffs[DungeonBuffManager.BUFF_CONFUSED].useDungeonBuff(arg);
	    } else if (!this.isConfused() && this.isRotated()) {
		return this.buffs[DungeonBuffManager.BUFF_ROTATED].useDungeonBuff(arg);
	    } else {
		return arg;
	    }
	}
