...
public class DungeonGUI {
	...
	this.generatorFrame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
	this.objectList = new DungeonObjectList();
	this.savedMapObject = (Tile) DungeonObjectList.getSpecificObject("Tile");
	...
	this.statsFrame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
	...
	this.viewingWindow = new Dimension(DungeonGUI.VIEWING_WINDOW_SIZE, DungeonGUI.VIEWING_WINDOW_SIZE);
    }
	...
	final NDimensionalLocation offset = new NDimensionalLocation(this.dungeon.getMaxDimension());
	offset.setLocation(NDimensionalMap.ROW_DIMENSION, y);
	offset.setLocation(NDimensionalMap.COLUMN_DIMENSION, x);
	final MapObject there = this.dungeon.getCellOffset(this.playerLocation, offset, fallback);
	...
	if (!there.isSolid()) {
	    ...
	    this.playerLocation.setLocation(NDimensionalMap.ROW_DIMENSION,
		    this.playerLocation.getLocation(NDimensionalMap.ROW_DIMENSION) + this.moveY);
	    this.playerLocation.setLocation(NDimensionalMap.COLUMN_DIMENSION,
		    this.playerLocation.getLocation(NDimensionalMap.COLUMN_DIMENSION) + this.moveX);
	    this.savedMapObject = (DungeonObject) there;
	    this.dungeon.setCell(player, this.playerLocation);
	    final Point newViewingWindowOffset = new Point(this.moveY, this.moveX);
	    this.dungeon.alterViewingWindowOffset(newViewingWindowOffset);
	    this.redrawDungeon();
	}
    }

    public void updateMonsterPosition(final int xMove, final int yMove, final int xLoc, final int yLoc,
	    final Monster monster) {
	Wall fallback = new Wall();
	final NDimensionalLocation loc = new NDimensionalLocation(this.dungeon.getMaxDimension());
	...
	final MapObject there = this.dungeon.getCellOffset(loc, offset, fallback);
	if (!there.isSolid() && !there.getName().equals("Monster")) {
	    this.dungeon.setCell(monster.getSavedObject(), loc);
	    loc.setLocation(NDimensionalMap.ROW_DIMENSION, loc.getLocation(NDimensionalMap.ROW_DIMENSION) + yMove);
		...
		if (there.getName().equals("Ice")) {
		    ...
		    if (DungeonDiver.getHoldingBag().getPrefs().getPreferenceValue(Preferences.ICE_ENABLED)) {
			// Yes it is - move the monster again
			this.updateMonsterPosition(xMove, yMove, xLoc + xMove, yLoc + yMove, monster);
		    }
		}
	    }
	}
	...
	if (this.outputFrame.isVisible()) {
	    this.borderPane.removeAll();
	    this.outputPane = this.dungeon.drawGameWithVisibilityBypassHook(null);
	    this.borderPane.add(this.outputPane, BorderLayout.CENTER);
	    this.borderPane.add(this.effectsPane, BorderLayout.SOUTH);
	    this.outputFrame.pack();
	    this.showDungeon();
	...
	if (this.generatorTask != null && this.outputFrame != null && this.statsFrame != null) {
    ...
    public void postBattle(final NDimensionalLocation loc, final Monster m, final boolean player) {
	...
	final int dl = DungeonDiver.getHoldingBag().getPlayer().getDungeonLevel();
	return DungeonGUI.DUNGEON_SIZE + (dl - 1) * DungeonGUI.DUNGEON_SIZE_INCREMENT;
    }
}