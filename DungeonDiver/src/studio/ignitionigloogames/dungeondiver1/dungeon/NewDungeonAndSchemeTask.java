...
public class NewDungeonAndSchemeTask implements Runnable {
	...
	try {
	    ...
	    final Darkness dark = (Darkness) DungeonObjectList.getSpecificObject("Darkness");
	    final int size = gui.computeDungeonSize();
	    final DungeonMap dungeon = new DungeonMap(size, size, viewingWindow, dark, dark);
	    final JFrame generatorFrame = gui.getGenerator();
	    gui.hideDungeon();
	    final Tile tile = (Tile) DungeonObjectList.getSpecificObject("Tile");
	    final Player player = (Player) DungeonObjectList.getSpecificObject("Player");
		...
		final Point newViewingWindow = new Point(
			playerLocation.getLocation(NDimensionalMap.ROW_DIMENSION) - DungeonGUI.VIEWING_WINDOW_SIZE / 2,
			playerLocation.getLocation(NDimensionalMap.COLUMN_DIMENSION)
