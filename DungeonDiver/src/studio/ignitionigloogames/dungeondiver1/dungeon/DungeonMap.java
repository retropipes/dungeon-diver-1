...
public class DungeonMap extends Map2D {
    ...
    // Constructors
    public DungeonMap(final int rows, final int cols, final Dimension viewingWindowDimensions,
    ...
    public Container drawGameWithVisibilityBypassHook(final NDimensionalLocation otherDimensions) {
	return super.drawGameWithVisibility(otherDimensions);
    }
	...
	final int columns = dimensions.getLocation(NDimensionalMap.COLUMN_DIMENSION);
	int x, y;
	for (x = 0; x < columns; x++) {
	    for (y = 0; y < rows; y++) {
