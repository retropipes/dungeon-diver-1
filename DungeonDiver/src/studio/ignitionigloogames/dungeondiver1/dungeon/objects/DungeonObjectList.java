...
import studio.ignitionigloogames.dungeondiver1.utilities.MapObjectList;

public final class DungeonObjectList extends MapObjectList {
    // Fields
    private static final DungeonObject[] objects = { new Armor(), new Bank(), new ClockwiseRotationTrap(),
	    new ConfusionTrap(), new CounterclockwiseRotationTrap(), new DarkGem(), new Darkness(), new Door(),
	    new Healer(), new Ice(), new LightGem(), new Monster(), new Player(), new Regenerator(), new StairsUp(),
	    new StairsDown(), new Switcher(), new SpinnerTrap(), new Tile(), new Wall(), new WarpTrap(), new Weapons(),
	    new ZoomTube() };
	...
	for (x = 0; x < DungeonObjectList.objects.length; x++) {
		...
		if (DungeonObjectList.objects[x].shouldCache()) {
		    ...
		    return MapObjectList.getNewInstance(DungeonObjectList.objects[x]);
		}
	    }
	}
	return null;
