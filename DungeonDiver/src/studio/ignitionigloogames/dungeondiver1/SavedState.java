...
public class SavedState {
	...
	if (System.getProperty("os.name").startsWith("Mac OS X")) {
	    final FileDialog fd = new FileDialog(DungeonDiver.getHoldingBag().getGUIManager().getParentFrame(), "Load",
		    FileDialog.LOAD);
	...
	try (ObjectInputStream stream = new ObjectInputStream(new FileInputStream(filename))) {
	    ...
	    temp.loadPlayer(pAttack, pDefense, pHP, pMP, newKills, newLevel, chp, cmp, newWeapon, newArmor, newGold,
		    newBank, newExperience, dl, bookID);
	    final String checksumData = SavedState.getChecksum(temp);
	    if (!checksumFile.equals(checksumData)) {
		JOptionPane.showMessageDialog(null, "Cheat attempt detected.", "Load State",
			JOptionPane.INFORMATION_MESSAGE);
	    } else {
		p = ClassManager.getNewPlayerInstance(bookID);
		p.loadPlayer(pAttack, pDefense, pHP, pMP, newKills, newLevel, chp, cmp, newWeapon, newArmor, newGold,
			newBank, newExperience, dl, bookID);
		DungeonDiver.getHoldingBag().setPlayer(p);
		result = true;
	    }
	} catch (final IOException ie) {
	    JOptionPane.showMessageDialog(null, "State could not be loaded.", "Load State",
		    JOptionPane.INFORMATION_MESSAGE);
	} catch (final Exception ex) {
	    ex.printStackTrace();
	    JOptionPane.showMessageDialog(null, "Unknown error reading state file.", "Load State",
		    JOptionPane.INFORMATION_MESSAGE);
	}
	return result;
    }
	...
	String filename, extension;
	if (System.getProperty("os.name").startsWith("Mac OS X")) {
	    final FileDialog fd = new FileDialog(DungeonDiver.getHoldingBag().getGUIManager().getParentFrame(), "Save",
		    FileDialog.SAVE);
	    final SavedStateFilterMacOSX ss = new SavedStateFilterMacOSX();
	    fd.setFilenameFilter(ss);
	    fd.setVisible(true);
		...
		if (extension != null) {
			...
			filename = SavedState.getNameWithoutExtension(filename)
				+ SavedStateIdentifier.getIdentifierWithPeriod();
		    }
		} else {
	...
	try (ObjectOutputStream stream = new ObjectOutputStream(new FileOutputStream(filename))) {
	    ...
	    JOptionPane.showMessageDialog(null, "State could not be saved.", "Save State",
		    JOptionPane.INFORMATION_MESSAGE);
	} catch (final Exception ex) {
	    JOptionPane.showMessageDialog(null, "Unknown error writing state file.", "Save State",
		    JOptionPane.INFORMATION_MESSAGE);
	}
    }
	...
	final String checksum = Long.toString(dl + 2 * (newExperience + 1) + 3 * ((long) newBank + 1)
		+ 4 * ((long) newGold + 1) + 5 * ((long) newArmor + 1) + 6 * ((long) newWeapon + 1)
		+ 7 * ((long) chp + 1) + 8 * ((long) cmp + 1) + 9 * ((long) newLevel + 1) + 10 * ((long) newKills + 1)
		+ 11 * ((long) pMP + 1) + 12 * ((long) pHP + 1) + 13 * ((long) pDefense + 1) + 14 * ((long) pAttack + 1)
		+ 15 * ((long) bookID + 1), SavedState.CHECKSUM_RADIX_2);
	return checksum;
    }
}
